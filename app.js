...
// var leftOneImg = document.getElementById('imgFirst');
...
//     if (trials !== 0) {
//         if (targetAny === 'imgFirst' || targetAny === 'two' || targetAny === 'three') {
...
function random() {
    do {
        var leftImage = Math.round(Math.random() * (arrayOfimg.length - 1));
        var left = arrayOfimg[leftImage].url;
    } while (makeSure(left));

    do {
        var centerImage = Math.round(Math.random() * (arrayOfimg.length - 1))
        var center = arrayOfimg[centerImage].url;
    } while (makeSure(center));
    do {
        var rightImage = Math.round(Math.random() * (arrayOfimg.length - 1))
